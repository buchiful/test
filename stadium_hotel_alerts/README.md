...
5. **連泊確認**: 条件に合致した物件について、3/13〜・3/14〜のチェックインでも
   予約可能かを追加で検索し、必須期間(3/15〜3/17)を含む最大連続宿泊数を
   メールに表示(連泊可能日数が増えた場合も再通知)
6. **通知**: 新しく見つかった物件(または価格が5%以上変動した物件)だけをメール送信。通知済みは `state.json` に記録して重複通知を防止
7. **定期実行**: GitHub Actions が6時間ごとに自動実行(`.github/workflows/hotel-alerts.yml`)
