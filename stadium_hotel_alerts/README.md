...
| 除外 | `config.yaml` の `filters.excluded_names` に列挙した物件名(部分一致)はヒットしても通知しない |
...
4. **フィルタ**: 予算・評価 3.7 以上・エアコン・30分圏内・除外リストで絞り込み(物件名が
   `excluded_names` のいずれかと部分一致すれば、他の条件を満たしても通知対象から除外)
